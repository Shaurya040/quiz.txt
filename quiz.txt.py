...
def start_quiz():
    global current_round, current_question, user_score, highest_score, round_cleared
...
def next_question():
    global current_round, current_question, user_score, highest_score, round_cleared
    if current_round < len([Round1_data, Round2_data, Round3_data, Round4_data]):
        if current_question < len([Round1_data, Round2_data, Round3_data, Round4_data][current_round]["questions"]):
            check_ans()
            user_ans.set('')
            question = list([Round1_data, Round2_data, Round3_data, Round4_data][current_round]["questions"].keys())[current_question]
            clear_frame()
            Label(f1, text=f"Question : {question}", padx=15, font="calibre 12 normal").pack(anchor=NW)
            for option in [Round1_data, Round2_data, Round3_data, Round4_data][current_round]["questions"][question]:
                Radiobutton(f1, text=option, variable=user_ans, value=option, padx=28).pack(anchor=NW)
            current_question += 1
        else:
            current_question = 0
            current_round += 1
            Label(f1, text=f"Round {current_round} Score: {user_score}/3", font="calibre 12 bold").pack()
...
def check_ans():
    ...
    if current_question > 0:
        ...
        correct_ans = [Round1_data, Round2_data, Round3_data, Round4_data][current_round]["answers"][
            current_question - 1]
